...
export async function main(ns) {
	...
	if (!ns.getPlayer().hasCorporation) {
		ns.corporation.createCorporation("GrottoInc");
	}
...
async function hireEmployees(ns, division, productCity = "Sector-12") {
  var divData = ns.corporation.getDivision(division)
	var employeeData = ns.corporation.getOffice(division, productCity)
  var employees = employeeData.numEmployees
  ns.print(employees)
	while (ns.corporation.getCorporation().funds > (divData.cities.length * ns.corporation.getOfficeSizeUpgradeCost(division, productCity, 3))) {
  ...
  // var employeeData = ns.corporation.getOffice(division, productCity)
	// if (employeeData.numEmployees > employees) {
	// 	// set jobs after hiring people just in case we hire lots of people at once and setting jobs is slow

		for (const city of divData.cities) {
      var employeeData = ns.corporation.getOffice(division, productCity)
			employees = employeeData.numEmployees;
			if (ns.corporation.hasResearched(division, "Market-TA.II")) {
				// TODO: Simplify here. ProductCity config can always be used
				if (city == productCity) {
					await ns.corporation.setJobAssignment(division, city, "Operations", Math.ceil(employees / 5));
					await ns.corporation.setJobAssignment(division, city, "Engineer", Math.ceil(employees / 5));
					await ns.corporation.setJobAssignment(division, city, "Business", Math.ceil(employees / 5));
					await ns.corporation.setJobAssignment(division, city, "Management", Math.ceil(employees / 10));
					var remainingEmployees = employees - (3 * Math.ceil(employees / 5) + Math.ceil(employees / 10));
					await ns.corporation.setJobAssignment(division, city, "Training", Math.ceil(remainingEmployees));
				}
				else {
					await ns.corporation.setJobAssignment(division, city, "Operations", Math.floor(employees / 10));
					await ns.corporation.setJobAssignment(division, city, "Engineer", 1);
					await ns.corporation.setJobAssignment(division, city, "Business", Math.floor(employees / 5));
					await ns.corporation.setJobAssignment(division, city, "Management", Math.ceil(employees / 100));
					await ns.corporation.setJobAssignment(division, city, "Research & Development", Math.ceil(employees / 2));
					...
					await ns.corporation.setJobAssignment(division, city, "Training", Math.floor(remainingEmployees));
				}
			}
			else {
				if (city == productCity) {
					await ns.corporation.setJobAssignment(division, city, "Operations", Math.floor((employees - 2) / 2));
					await ns.corporation.setJobAssignment(division, city, "Engineer", Math.ceil((employees - 2) / 2));
					await ns.corporation.setJobAssignment(division, city, "Management", 2);
				}
				else {
					await ns.corporation.setJobAssignment(division, city, "Operations", 1);
					await ns.corporation.setJobAssignment(division, city, "Engineer", 1);
					await ns.corporation.setJobAssignment(division, city, "Research & Development", (employees - 2));
				}
			}
		}
	// }
...
async function trickInvest(ns, division, productCity = "Sector-12") {
	...
	for (const city of cities) {
		...
		await ns.corporation.setJobAssignment(division, city, "Engineer", 0);
		await ns.corporation.setJobAssignment(division, city, "Management", 0);
		await ns.corporation.setJobAssignment(division, city, "Research & Development", 0);
		await ns.corporation.setJobAssignment(division, city, "Operations", employees - 2); // workaround for bug
		await ns.corporation.setJobAssignment(division, city, "Operations", employees - 1); // workaround for bug
		await ns.corporation.setJobAssignment(division, city, "Operations", employees);
	}
	...
	for (const city of cities) {
		...
		const employees = ns.corporation.getOffice(division, city).employees.length;
		await ns.corporation.setJobAssignment(division, city, "Operations", 0);
		await ns.corporation.setJobAssignment(division, city, "Business", employees - 2); // workaround for bug
		await ns.corporation.setJobAssignment(division, city, "Business", employees - 1); // workaround for bug
		await ns.corporation.setJobAssignment(division, city, "Business", employees);
	}
	for (var product of division.products) {
		// sell products again
	...
	for (const city of cities) {
		...
		const employees = ns.corporation.getOffice(division, city).employees.length;
		await ns.corporation.setJobAssignment(division, city, "Business", 0);
		if (city == productCity) {
			await ns.corporation.setJobAssignment(division, city, "Operations", 1);
			await ns.corporation.setJobAssignment(division, city, "Engineer", (employees - 2));
			await ns.corporation.setJobAssignment(division, city, "Management", 1);
		}
		else {
			await ns.corporation.setJobAssignment(division, city, "Operations", 1);
			await ns.corporation.setJobAssignment(division, city, "Research & Development", (employees - 1));
		}
	}
...
function doResearch(ns, division) {
	...
	if (!ns.corporation.hasResearched(division, laboratory)) {
		...
		for (const researchObject of researchList) {
			...
			if (!ns.corporation.hasResearched(division, researchObject)) {
				if (division.research > (researchObject.prio * ns.corporation.getResearchCost(division, researchObject))) {
					ns.print(division + " Research " + researchObject);
					ns.corporation.research(division, researchObject);
				}
			}
		}
	}
...
async function initCities(ns, division, productCity = "Sector-12") {
	for (const city of cities) {
		...
		if (city != productCity) {
			...
			await ns.corporation.setJobAssignment(division, city, "Research & Development", 3);
		}
		else {
			...
			await ns.corporation.setJobAssignment(division, productCity, "Operations", 4);
			await ns.corporation.setJobAssignment(division, productCity, "Engineer", 6);
			await ns.corporation.setJobAssignment(division, productCity, "Management", 2);
		}
		const warehouseUpgrades = 3;
		for (let i = 0; i < warehouseUpgrades; i++) {
			ns.corporation.upgradeWarehouse(division, city);
...
async function initialCorpUpgrade(ns) {
  ...
  if (!ns.corporation.hasUnlock("Smart Supply")){
	...
	// 
  var corpPlayer = ns.corporation.getCorporation()    
  while (ns.corporation.getUpgradeLevel("Smart Storage") < 4){
    var upgradecost = await ns.corporation.getUpgradeLevelCost("Smart Storage")
    if (upgradecost <= corpPlayer.funds){
	    ns.corporation.levelUpgrade("Smart Storage");
    }
    await ns.sleep(1000)
    corpPlayer = ns.corporation.getCorporation()
	  ns.print("Awaiting Upgrade for ", "Smart Storage");
  }
	// 
  while (ns.corporation.getUpgradeLevel("Nuoptimal Nootropic Injector Implants") < 1){
    if (ns.corporation.getUpgradeLevelCost("Nuoptimal Nootropic Injector Implants") < corpPlayer.funds){
	    ns.corporation.levelUpgrade("Nuoptimal Nootropic Injector Implants");
    }
    await ns.sleep(1000)
    corpPlayer = ns.corporation.getCorporation()
	  ns.print("Awaiting Upgrade for ", "Nuoptimal Nootropic Injector Implants");
  }
	// 
  while (ns.corporation.getUpgradeLevel("Speech Processor Implants") < 1){
    if (ns.corporation.getUpgradeLevelCost("Speech Processor Implants") < corpPlayer.funds){
	    ns.corporation.levelUpgrade("Speech Processor Implants");
    }
    await ns.sleep(1000)
    corpPlayer = ns.corporation.getCorporation()
	  ns.print("Awaiting Upgrade for ", "Speech Processor Implants");
  }
	// 
  while (ns.corporation.getUpgradeLevel("Neural Accelerators") < 1){
    if (ns.corporation.getUpgradeLevelCost("Neural Accelerators") < corpPlayer.funds){
	    ns.corporation.levelUpgrade("Neural Accelerators");
    }
    await ns.sleep(1000)
    corpPlayer = ns.corporation.getCorporation()
	  ns.print("Awaiting Upgrade for ", "Neural Accelerators");
  }
	// 
  while (ns.corporation.getUpgradeLevel("FocusWires") < 1){
    if (ns.corporation.getUpgradeLevelCost("FocusWires") < corpPlayer.funds){
	    ns.corporation.levelUpgrade("FocusWires");
    }
    await ns.sleep(1000)
    corpPlayer = ns.corporation.getCorporation()
	  ns.print("Awaiting Upgrade for ", "FocusWires");
