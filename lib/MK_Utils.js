...
export async function scanNetworkForNonPurchasedServers(ns, host) {  
  let outcome = ns.scan(host)
  var targetList = [];
  for (var servername of Object.keys(outcome)){
    let prefix = outcome[servername].substr(0, 3);
    if (prefix != 'mk_'){
      targetList.push(outcome[servername])
    }
  }
  return targetList;
}
