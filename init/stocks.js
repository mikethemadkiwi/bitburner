...
async function trader(ns) {
    ...
    const debugPrint = (sym, isLong) => {
        ...
        if (isLong != undefined) {
            symStr = (isLong ? '📈': '📉') + sym
            endStr = ` 💲${ns.format.number(investment, '000.00a')} 💲${ns.format.number(income, '000.00a')} ${income > 0 ? '👍' : '👎'}`
        }
        ns.print(`${symStr}\t${stock.getForecast(sym).toFixed(2)} - ${(stock.getVolatility(sym) * 100).toFixed(2)} ${ns.format.number(getSymbolPoint(stock, sym), '00.00')}` + endStr)
    }
...
const dateSuffix = (ns) => `${ns.format.number(new Date().getMonth(), '00')}-${ns.format.number(new Date().getDate(), '00')}`
/** @param {NS} ns */
...
async function logBuySell(ns, symObj, isBuy) {
    ...
    const date = `${ns.format.number(dateObj.getHours(), '00')}:${ns.format.number(dateObj.getMinutes(), '00')}`
...
function printLogs(ns) {
    ...
    ns.print(`On Market:     💲${ns.format.number(onMarketValue, '0.0a')}(+💲${ns.format.number(onMarketValueChange, '0.0a')})`)
    ns.print(`Earned(today): 💲${ns.format.number(earnedMoney, '0.0a')}`)
    ns.print('INFO\tBUY/SELL LOG')
    // const date = new Date()
    for (let log of logs) {
        const { sym, long, short, isBuy, profit, date } = log
        const operation = isBuy ? 'BUY ' : 'SELL'
        const amount = short > 0 ? `SHORT ${ns.format.number(short, '000.0a')}` : `LONG  ${ns.format.number(long, '000.0a')}`
        const profitStr = profit ? `💲${ns.format.number(profit, '0.0a')}` : ''
