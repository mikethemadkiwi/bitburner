...
export async function main(ns) {
  ...
  var factionServers = ['CSEC', 'avmnite-02h', 'I.I.I.I', 'run4theh111z'];
  var seenServerList = [];
  ...
  async function fitScripts(){
    ...
    threadscanfit = (homeFreeRam/ns.getScriptRam(localAttack))
    if (threadscanfit > 1){
      await ns.run(localAttack, Math.floor(threadscanfit), 'n00dles');
    }
  }
          ...
          if (!isInBlacklist(tarHost)){
            if (hasCracksforPorts(reqPorts)){
              ...
              if (isAFactionServer(tarHost)){
                // await ns.singularity.backdoor(tarHost)
              }
        ...
        if (!isInBlacklist(tarHost)){
          ...
          if (hasRam(ns.getServerMaxRam(tarHost)), localAttack){
            if (!isInSTL(tarHost)){
              ...
              if (maxThreads>=1) {
                ns.exec(localAttack, tarHost, maxThreads, 'n00dles', 0, false)
